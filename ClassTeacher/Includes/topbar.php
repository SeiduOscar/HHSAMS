...
<?php 
  ...
  $query = "SELECT * FROM tblmoderator WHERE Id = ".$_SESSION['userId']."";
  $rs = $conn->query($query);
  $num = $rs->num_rows;
  $rows = $rs->fetch_assoc();
...
<nav class="navbar navbar-expand navbar-light bg-gradient-primary topbar mb-4 static-top">
          <button class="btn btn-link rounded-circle mr-3 d-lg-none" type="button"
        data-toggle="collapse" data-target="#sidebarWrapper"
        aria-controls="sidebarWrapper" aria-expanded="false" aria-label="Toggle sidebar">
